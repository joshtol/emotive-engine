...
class CustomGesturePlugin {
    ...
    applyWobbleToParticle(particle, animation, progress) {
        ...
        const wobbleOffset =
            Math.sin(easedProgress * Math.PI * 2 * frequency) * amplitude * (1 - progress); // Decay over time
    ...
    updateWobble(animation, progress) {
        ...
        if (this.mascot.renderer?.state) {
            if (axis === 'horizontal') {
                this.mascot.renderer.state.gazeOffset.x =
                    animation.initialPosition.x + wobbleOffset;
            } else {
                this.mascot.renderer.state.gazeOffset.y =
                    animation.initialPosition.y + wobbleOffset;
            }
        }
    }
    ...
    updateHeartbeat(animation, progress) {
        ...
        if (elapsed < beatDuration) {
            ...
            scale =
                ...
                (firstPulse - animation.initialScale) * (1 - Math.abs(easedProgress * 2 - 1));
        } else if (elapsed < beatDuration + pauseBetween) {
            // Pause between beats
            ...
            scale =
                animation.initialScale +
                (secondPulse - animation.initialScale) * (1 - Math.abs(easedProgress * 2 - 1));
        }
