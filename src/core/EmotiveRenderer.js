...
class EmotiveRenderer {
    ...
    getEffectiveCenter() {
        ...
        if (this.config.positionController) {
            effectiveCenter = this.config.positionController.getEffectiveCenter(
                baseCenter.x,
                baseCenter.y
            );
        } else {
    ...
    render(state, deltaTime, gestureTransform = null) {
        ...
        gestureTransform = this.transformMerger.mergeAndStoreTransforms(
            gestureTransform,
            deltaTime
        );
        ...
        this.canvasSetupManager.performCanvasSetup(
            logicalWidth,
            logicalHeight,
            originalCtx,
            deltaTime
        ...
        const dims = this.dimensionCalculator.calculateRenderDimensions(
            logicalWidth,
            logicalHeight,
            state,
            gestureTransform
        ...
        const radiusData = this.radiusCalculator.calculateRadii(
            state,
            scaleMultiplier,
            glowMultiplier
        );
        ...
        const totalRotation = this.rotationManager.applyRotationTransform(
            this.ctx,
            coreX,
            coreY,
            rotationAngle
    ...
    renderRecordingGlow(x, y, radius, intensity) {
        ...
        const gradient = gradientCache.getRadialGradient(this.ctx, x, y, 0, x, y, safeRadius, [
    ...
    renderDropShadow(x, y, radius, shapePoints) {
        ...
        const hasAudioDeformation =
            this.shapeMorpher &&
            (this.shapeMorpher.audioDeformation > 0.1 || this.shapeMorpher.vocalEnergy > 0.1);
    ...
    startGrooveSway(options) {
        this.ambientDanceAnimator.startAmbientAnimation('grooveSway', options);
    }
    startGrooveBob(options) {
        this.ambientDanceAnimator.startAmbientAnimation('grooveBob', options);
    }
    startGrooveFlow(options) {
        this.ambientDanceAnimator.startAmbientAnimation('grooveFlow', options);
    }
    startGroovePulse(options) {
        this.ambientDanceAnimator.startAmbientAnimation('groovePulse', options);
    }
    startGrooveStep(options) {
        this.ambientDanceAnimator.startAmbientAnimation('grooveStep', options);
    }
